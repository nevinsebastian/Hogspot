...
const CustomTabBar = ({ state, descriptors, navigation }) => {
  ...
  const handleUploadPhoto = () => {
    // Navigate to UploadPhoto screen in the parent stack navigator
    const parent = navigation.getParent();
    if (parent) {
      parent.navigate('UploadPhoto');
    }
  };

  return (
    <View style={[styles.bottomNavbar, { bottom: Math.max(insets.bottom, 24) }]}>
      {state.routes && state.routes.map((route, index) => {
        ...
        const screenName = getScreenName(route.name);
        ...
        // Insert center button after the second tab (Discover)
        const shouldShowCenterButton = index === 1;
          ...
          <React.Fragment key={route.key}>
            ...
            >
              <View style={[styles.iconContainer, isFocused && styles.activeIconContainer]}>
                <SvgXml 
                  xml={icon} 
                  style={[styles.navIcon, isFocused && styles.activeNavIcon]} 
                />
              </View>
            </TouchableOpacity>
            
            {/* Center Plus Button after Discover tab */}
            {shouldShowCenterButton && (
              <TouchableOpacity onPress={handleUploadPhoto} style={styles.centerButton}>
